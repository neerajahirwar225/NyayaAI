...
const Register = () => {
  ...
  return (
    <div>
      <h3 className="text-xl font-bold text-white text-center mb-6">Create Account</h3>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        {/* Full Name */}
        <div>
          <label className="block text-xs font-semibold text-brand-textMuted uppercase tracking-wider mb-2">Full Name</label>
          <input
            type="text"
            placeholder="John Doe"
            className={`w-full px-4 py-3 rounded-xl glass-input text-white text-sm ${errors.name ? 'border-brand-danger/50 focus:border-brand-danger/50' : ''}`}
        ...
        {/* Confirm Password */}
        <div>
          <label className="block text-xs font-semibold text-brand-textMuted uppercase tracking-wider mb-2">Confirm Password</label>
          <input
            type="password"
            placeholder="••••••••"
            className={`w-full px-4 py-3 rounded-xl glass-input text-white text-sm ${errors.confirmPassword ? 'border-brand-danger/50 focus:border-brand-danger/50' : ''}`}
      ...
      <div className="mt-6 text-center text-sm text-brand-textMuted">
        ...
        <Link to="/login" className="text-brand-accentLight hover:underline font-semibold">
          Log in
        </Link>
      </div>
    </div>
